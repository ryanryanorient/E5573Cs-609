...
/******************************************************************************
修改日期        修改人        问题单号                 简要描述
2012/07/03      l00182072     DTS2012063002727         连接被disable后，删除连接超时函数
2012/07/07      l00182072     DTS2012070307199         添加下发最大扫描个数的接口
2012/08/25      l00182072     DTS2012082005716         在event_disassoc中添加上报断开
2012/09/26      l00182072     DTS2012082707533         STA WPS连接成功后，下次自动连接成功
2012/09/26      l00182072     DTS2012063005057         添加序号解决supplicant消息乱序问题  
2012/11/09      l00182072     DTS2012111300780         支持EAP-SIM特性
2012/11/26      l00182072     DTS2012112006806         解决WPS PBC问题
2013/05/28      s00208871     DTS2013052902934         合入指定信道扫描功能，参见问题单DTS2013032100903
2013/07/12      s00208871     DTS2013061809061         根据scan提供的信道集合对扫描结果进行过滤
2013/10/08      s00208871     DTS2013100300345         断开次数统计不准确
2013/10/23      s00208871     DTS2013102303890         station wps时，上报对端的加密认证信息
2013/11/20      s00208871     DTS2013111807435         wps成功后，上报信道信息
2014/03/29      s00208871     DTS2014012203040         添加多个profile时，手动添加一个隐藏SSID，概率出现无法连接成功，成功后高概率扫描不到已经连接的AP
******************************************************************************/
...
/* BEGIN PN: DTS2012070307199,Added  by l00182072, 2012/7/7*/
unsigned int g_scan_maxnum = CSCAN_MAXNUM;
/* END   PN: DTS2012070307199,Added  by l00182072, 2012/7/7*/

/* BEGIN PN: DTS2012082707533,Added  by l00182072, 2012/9/3*/
#ifdef CONFIG_WPS
unsigned int g_wps_auth_type = 0;
int g_wpa_freq = 0;
#endif
/* END   PN: DTS2012082707533,Added  by l00182072, 2012/9/3*/
...
/* BEGIN PN: DTS2013102303890,Added  by s00208871 , 2013/10/23*/
WPA_SUPPLICANT_PRIVATE_STATE g_wpa_last_state = WPA_SUPPLICANT_PRV_DISCONNECTED;
/* END PN: DTS2013102303890,Added  by s00208871, 2013/10/23*/
...
/*BEGIN PN: DTS2013052902934  2013/05/24  by shenxiong    ADD */
...
/*END PN: DTS2013052902934  2013/05/24  by shenxiong    ADD */
/* BEGIN PN: DTS2013061809061 ,Added by s00208871, 2013/7/4 */
extern void dump_scan_channel(struct scan_channel *channels);
/* END PN: DTS2013052902934 ,Added by s00208871, 2013/7/4 */
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		/* BEGIN PN: DTS2012112006806, Modified by l00182072, 2012/11/26*/
    ...
    gWpaSupplicantCurrentState = WPA_SUPPLICANT_PRV_WPSPBCING;
    /* END   PN: DTS2012082005716, Modified by l00182072, 2012/8/21*/
    ...
    gWpaSupplicantCurrentState = WPA_SUPPLICANT_PRV_WPSPBCING;
		reply_len = wpa_supplicant_ctrl_iface_wps_pin(wpa_s, buf + 8,
							      reply,
							      reply_size);
    /* END   PN: DTS2012112006806, Modified by l00182072, 2012/11/26*/
	} else if (os_strncmp(buf, "WPS_CHECK_PIN ", 14) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_wps_check_pin(
			wpa_s, buf + 14, reply, reply_size);
	} else if (os_strcmp(buf, "WPS_CANCEL") == 0) {
    /* BEGIN PN: DTS2012082005716, Modified by l00182072, 2012/8/21*/
    ...
    /* END PN: DTS2013061809061 ,Added by s00208871, 2013/7/4 */
#ifdef ANDROID
    ...
    /*BEGIN DTS2013100300345   s00208871   2013-10-08    Modified*/
    wpa_printf(MSG_DEBUG, "current scan times:%d\n", g_ScanResultsTimes);
    g_ScanResultsTimes = 0;
    /*END DTS2013100300345   s00208871   2013-10-08    Modified*/

...
2013/10/22      s00208871     DTS2013102300631         调整assoc prefer的执行位置，从stacall模块，移至wpa_supplicant.解决offload连接成功后，配参数时fw异常
2013/11/20      s00208871     DTS2013111807435         记录wps成功后使用的信道值
2014/01/07      s00208871     DTS2014010206112         输入正确密码进行连接，长时间连接后，没有连接成功，弹出密码框要求重新输入密码
2014/03/17      s00208871     DTS2014031708408         根据代码review对扫描进行优化
2014/03/29      s00208871     DTS2014012203040         添加多个profile时，手动添加一个隐藏SSID，概率出现无法连接成功，成功后高概率扫描不到已经连接的AP
2014/05/29      y00187730     DTS2014052705641         博通89335芯片不使用信道过滤及combo scan的功能
...
int wpa_driver_wext_combo_scan(void *priv, struct wpa_driver_scan_params *params)
{
  ...
  /*BEGIN DTS2013052902934 shenxiong 2013-05-28 added*/
  bp += wext_set_chan_list(buf + bp, &wpa_s->channels);
  /*END DTS2013052902934 shenxiong 2013-05-28 added*/
	if (MULTI_UNICAST_SSID_SCAN_FLAG == wpa_s->scan_flag||
    MULTI_UNICAST_BROADCAST_SSID_SCAN_FLAG == wpa_s->scan_flag) {
...
struct wpa_scan_results * wpa_driver_wext_get_scan_results(void *priv)
{
  ...
  /* BEGIN PN: DTS2013061809061 ,Added by s00208871, 2013/07/4*/ 
  struct wpa_supplicant *wpa_s = (struct wpa_supplicant *)(drv->ctx);
  if(NULL != wpa_s) {
...
/*BEGIN DTS2013102300631 s00208871 2013-10-26 added*/
#ifdef FEATURE_MBB_WIFI_VSDB
...
#endif
/*END DTS2013102300631 s00208871 2013-10-26 added*/

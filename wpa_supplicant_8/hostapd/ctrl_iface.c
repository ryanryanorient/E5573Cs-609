...
/******************************************************************************
修改日期        修改人        问题单号                 简要描述
2014/07/18      s00208871     DTS2014071802655         在hostapd中完成消息封装并发送给wifi模块
******************************************************************************/
...
static void hostapd_ctrl_iface_send_ex(const char *buf, size_t len);
				      ...
				      const char *txt, size_t len)
{
	...
	if (hapd == NULL)
		return;
	hostapd_ctrl_iface_send_ex(txt, len);
				    ...
				    const char *buf, size_t len)
{
	...
	io[0].iov_len = 0;//os_strlen(levelstr); /*上报消息不带log级别 */
...
static void hostapd_ctrl_iface_send_ex(const char *buf, size_t len)
{
    WLAN_BACK_MSG         wlan_msg;  
    static int            flag_wps_done = 0;   /* 用来标识WPS是否协商结束 */
    char msg[128];
    int ret = 0;

    if(len <= 0 || len > sizeof(msg)) {
        wpa_printf(MSG_ERROR, "bad buf length:%d", len);
        return;
    }    
    wpa_printf(MSG_INFO, "info:%s, len:%d", buf, len);

    /* 初始化为一个无效消息 */    
    os_memset(&wlan_msg, 0, sizeof(wlan_msg));
    wlan_msg.WlanMSGType = WLAN_MAX_IND;

    os_memset(msg, 0, sizeof(msg));
    os_memcpy(msg, buf, len);
    if (0 == os_strncmp(msg, AP_STA_CONNECTED, strlen(AP_STA_CONNECTED)) 
      || 0 == os_strncmp(msg, AP_STA_DISCONNECTED, strlen(AP_STA_DISCONNECTED))) {
        wlan_msg.WlanMSGType = WLAN_LIST_UPDATE_IND;
    } 
    if (0 == os_strncmp(msg, WPA_EVENT_EAP_STARTED, strlen(WPA_EVENT_EAP_STARTED))) {
        flag_wps_done = 0; /* 复位 */
        wpa_printf(MSG_INFO, "WPS start");
        return;
    }
#if defined(FEATURE_HUAWEI_MBB_WIFIAPWPS)
    if (0 == os_strncmp(msg, WPS_EVENT_SUCCESS , strlen(WPS_EVENT_SUCCESS))) {
        flag_wps_done = 1; /* 置位 */
        wpa_printf(MSG_INFO, "WPS registration completed successfully");
        return;
    }
    if (0 == os_strncmp(msg, WPA_EVENT_EAP_FAILURE, strlen(WPA_EVENT_EAP_FAILURE))
      && 1 == flag_wps_done) {
        wpa_printf(MSG_INFO, "WPS Success");
        wlan_msg.WlanMSGType = WLAN_WPS_SUCCESS_IND;
    }
    if (0 == os_strncmp(msg, WPS_EVENT_TIMEOUT, strlen(WPS_EVENT_TIMEOUT))
      || 0 == os_strncmp(msg, WPS_EVENT_FAIL, strlen(WPS_EVENT_FAIL))
      || (0 == os_strncmp(msg, WPA_EVENT_EAP_FAILURE, strlen(WPA_EVENT_EAP_FAILURE)) && (0 == flag_wps_done)))
    {
        wpa_printf(MSG_INFO, "WPS Fail");
        wlan_msg.WlanMSGType = WLAN_WPS_FAILURE_IND;
    }
#endif
    wpa_printf(MSG_INFO, "WPS flag:%d, wlan msg type:%d", flag_wps_done, wlan_msg.WlanMSGType);
    if (WLAN_MAX_IND == wlan_msg.WlanMSGType) 
    {
        wpa_printf(MSG_WARNING, "unknow message");
        return;
    }
    
    ret = (int)IPC_CliMsgProcess(NPDAEMON_MSG_WLAN_FEEDBACK_MSG_PARA, &wlan_msg, sizeof(wlan_msg), 
                          IPC_MSG_MODE_ASYNC, 0, 0, IPC_CLIENT_RECV_TIMEOUT, IPC_NPDAEMON_NAME);
    wpa_printf(MSG_INFO, "[%s]:report event = %d, ret = %d\n", __FUNCTION__, (int)wlan_msg.WlanMSGType, ret);
    return;
}

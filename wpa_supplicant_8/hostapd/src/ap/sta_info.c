...
/******************************************************************************
修改日期        修改人        问题单号                 简要描述
2014/06/13      s00208871     DTS2014061305053        wep和open方式下上报连接和断开消息
******************************************************************************/
...
void ap_free_sta(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	wpa_printf(MSG_INFO, "ieee802_1x:%d, wpa:%d", hapd->conf->ieee802_1x, hapd->conf->wpa);
	if (!hapd->conf->ieee802_1x && !hapd->conf->wpa) {
		char buf[100];
		wpa_printf(MSG_DEBUG, "send disconnect message\n");
...
struct sta_info * ap_sta_add(struct hostapd_data *hapd, const u8 *addr)
{
	...
	if (!hapd->conf->ieee802_1x && !hapd->conf->wpa) {
		char buf[100];
		wpa_printf(MSG_INFO, "send connected message\n");
		os_snprintf(buf, sizeof(buf), MACSTR, MAC2STR(sta->addr));

    ...
    http_encryption_t encryption)	/* I - Type of encryption to use */
{
  ...
  printf("entry httpConnectEncrypt(host=\"%s\", port=%d, encryption=%d)\n",
  ...
  */
  ...
  free(http);
  printf("Leave httpConnectEncrypt\n");
  return (NULL);
}
    ...
    int               family)		/* I - Address family or AF_UNSPEC */
{
  ...
  printf("entry httpCreate(host=\"%s\", port=%d, encryption=%d)\n",
                host, port, encryption);
...
{
  printf("entry httpReconnect(http=%p)\n", http);
  printf("Leave httpReconnect(http=%p)\n", http);
	       ...
	       int    *cancel)		/* I - Pointer to "cancel" variable */
{
  ...
  printf("entry httpReconnect2(http=%p, msec=%d, cancel=%p)\n", http, msec,
                cancel);
  ...
  if (http->tls)
  {
    printf("2httpReconnect2: Shutting down SSL/TLS...\n");
    http_shutdown_ssl(http);
  }
#endif /* HAVE_SSL */
  ...
  */
  ...
  if (http->fd >= 0)
  {
    printf("2httpReconnect2: Closing socket %d...\n", http->fd);
  ...
  */
  ...
  for (current = http->addrlist; current; current = current->next)
    printf("2httpReconnect2: Address %s:%d\n",
                               ...
                               cancel)) == NULL)
  {
    ...
    */
    ...
    printf("1httpReconnect2: httpAddrConnect failed: %s\n",
                  strerror(http->error));

    return (-1);
  }

  printf("2httpReconnect2: New socket=%d\n", http->fd);

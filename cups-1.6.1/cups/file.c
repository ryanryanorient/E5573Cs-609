...
//      (fileinfo.st_mode & S_IWOTH))	/* 4. Must not be writable by others */
  {
    if(fileinfo.st_uid)
		printf("fileinfo.st_uid true\n");
	if(fileinfo.st_mode & S_IWGRP)
		printf("fileinfo.st_mode & S_IWGRP true\n");
	if(fileinfo.st_mode & S_ISUID)
		printf("fileinfo.st_mode & S_ISUID true \n");
	if(fileinfo.st_mode & S_IWOTH)
		printf("fileinfo.st_mode & S_IWOTH true \n");
             ...
             const char *mode)		/* I - Open mode */
{
  ...
  printf("cupsFileOpen(filename=\"%s\", mode=\"%s\")", filename,
                mode);

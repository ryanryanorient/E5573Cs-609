     ...
     char *argv[])		/* I - Command-line arguments */
{
  ...
  system("busybox chmod 777 -R /dev/usb");
  for (i = 1; i < argc; i ++)
    if (argv[i][0] == '-')
      switch (argv[i][1])
      {
		...
		return (1);
	      ...
	      printf("printer = %s\n",printer);
            ...
            if (!validate_name(printer))
	    {
	      printf("lpadmin: Printer name can only contain "
			      "printable characters.");
	      return (1);
	    }
	    break;
    ...
    char          *file)		/* I - PPD file/interface script */
{
  ...
  if (NULL == http)
  {
      system("echo set_printer_options http is null >> /var/log/cups.log");
      return -1;
  }

  if (NULL == printer)
  {
      system("echo set_printer_options printer is null >> /var/log/cups.log");
      return -1;
  }

  if (NULL == options)
  {
      system("echo set_printer_options options is null >> /var/log/cups.log");
      return -1;
  }

  if (NULL == file)
  {
      system("echo set_printer_options file is null >> /var/log/cups.log");
  }
#if 0
  printf(("set_printer_options(http=%p, printer=\"%s\", num_options=%d, "
  ...
  * Add the options...
  */
  printf("Add the options name[%s],value[%s]\n",options->name,options->value);

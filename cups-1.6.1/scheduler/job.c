            ...
            const char *dest)		/* I - Job destination */
{
  ...
  printf("Into cupsdAddJob Function to Add a new job to the job queue with priority %d dest %s\n",priority,dest);
...
cupsdCheckJobs(void)
{
 ...
 //                 "curtime=%ld\n", cupsArrayCount(ActiveJobs), Sleeping,
 //                 NeedReload, (long)curtime);
       ...
       job = (cupsd_job_t *)cupsArrayNext(ActiveJobs))
  {
    ...
    printf("cupsdCheckJobs: Job %d - dest=\"%s\", printer=%p, "
                    "state=%d, cancel_time=%ld, hold_until=%ld, kill_time=%ld, "
    ...
    */
    ...
    if (job->kill_time && job->kill_time <= curtime)
    {
      cupsdLogMessage(CUPSD_LOG_ERROR, "[Job %d] Stopping unresponsive job.",
		      job->id);
      printf("[Job %d] Stopping unresponsive job.\n",
		      job->id);
    ...
    */
    ...
    if (job->cancel_time && job->cancel_time <= curtime)
    {
      ...
      printf("Canceling stuck job after %d seconds.\n", MaxJobTime);
      continue;
    }
	...
	*/
        printf("this job is pending,check every client\n");
	...
	for (con = (cupsd_client_t *)cupsArrayFirst(Clients);
      ...
      printf("Job submission timed out.\n");
    }
    ...
    */
	...
	((FilterLevel + job->pending_cost) < FilterLimit || FilterLevel == 0))
    {
    	  ...
    	  printf("Continue jobs that waiting on the FilterLimit\n");
    }
    ...
    */
        ...
        !DoingShutdown && !job->printer)
    {
      ...
      printf("cupsd start pending FindDest,name=[%s],uri=[%s],printer_types=[%04x],holding_new_jobs=[%d]\n",
        printer->name,printer->device_uri,printer->type,printer->holding_new_jobs);
	...
	*/
	 printf("printer class exist\n");

        pclass = printer;

        if (pclass->state == IPP_PRINTER_STOPPED)
	 {
	     printf("pclass stata return NULL\n");
	     printer = NULL;
        }
	...
	*/
        ...
        cupsdSetJobState(job, IPP_JOB_ABORTED, CUPSD_JOB_PURGE,
			 ...
			 "has gone away.");
	printf("Job aborted because the destination printer/class "
			 "has gone away.\n");
      }
	...
	*/
        ...
        if (pclass)
	{
	  ...
	  */
          printf("find which printer actually printed\n");
...
cupsdCleanJobs(void)
{
  ...
  printf("Into cupsdCleanJobs Function\n");
...
cupsdContinueJob(cupsd_job_t *job)	/* I - Job */
{
  ...
  printf("Into cupsdContinueJob Function cupsdContinueJob(job=%p(%d)): current_file=%d,num_files=%d\n",
	          job, job->id, job->current_file, job->num_files);
  ...
  */
  ...
  if (job->printer->raw)
  {
    ...
    */
    ...
    printf("Sending job to queue tagged as raw...\n");
  }
  else
  {
   /*
    ...
    */
    printf("Local jobs to get filetered\n");
    ...
    if (!filters)
    {
      cupsdLogJob(job, CUPSD_LOG_ERROR,
		  ...
		  job->current_file);
      printf("Unable to convert file %d to printable format. goto abort_job\n",
		  job->current_file);
      abort_message = "Aborting job because it cannot be printed.";
      abort_state   = IPP_JOB_ABORTED;

      ippSetString(job->attrs, &job->reasons, 0, "document-unprintable-error");
      printf("Aborting job because it cannot be printed.\n");
      goto abort_job;
    }
  ...
  */
      ...
      FilterLimit > 0)
  {
    ...
    */
    ...
    job->cost         = 0;
    printf("cupsdContinueJob: file=%d, cost=%d, level=%d, limit=%d\n",
		job->current_file, job->cost, FilterLevel,
		FilterLimit);
    return;
  }
       ...
       (job->num_files > 1 || !strncmp(job->printer->device_uri, "file:", 5))))
  {
    ...
    */
    ...
    if (!cupsArrayInsert(filters, &gziptoany_filter))
    {
      ...
      printf("Unable to add decompression filter - %s\n", strerror(errno));
      cupsArrayDelete(filters);

      abort_message = "Stopping job because the scheduler ran out of memory.";
      printf("Stopping job because the scheduler ran out of memory.\n");
      goto abort_job;
    }
  }
  ...
  if (job->printer->port_monitor)
  {
    ...
    */
    ...
    snprintf(port_monitor.filter, sizeof(port_monitor.filter),
     ...
     printf("port_monitor.filter=%s/monitor/%s\n", ServerBin, job->printer->port_monitor);
  ...
  */
  ...
  if (cupsArrayCount(filters) > MAX_FILTERS)
  {
    ...
    printf("Too many filters (%d > %d), unable to print.",
		cupsArrayCount(filters), MAX_FILTERS);
  ...
  */
  ...
  printf("\nBuild the command-line OK\n");

 /*
  * Create environment variable strings for the filters...
  */

 printf("+++++++++++++++++++begin create environment variable string\n");
  ...
  printf("charset = [%s],lang=[%s],ppd=[%s],rip_max_cache=[%s],content_type=[%s],device_uri=[%s],printer_info=[%s],printer_location=[%s],printer_name=[%s],printer_state_reasons[%s],banner_page=[%d],",
    charset,lang,ppd,rip_max_cache,content_type,device_uri,printer_info,printer_location,printer_name,printer_state_reasons,banner_page);
  ...
  if (!job->printer->remote && !job->printer->raw)
  {
	...
	snprintf(final_content_type, sizeof(final_content_type),
      ...
      printf("final_content_type=[%s],",final_content_type);
    }
  }
  ...
  printf("auth_info_required=[%s],",auth_info_required);
  ...
  for (i = 0;
    ...
    if (job->auth_env[i])
    {
        ...
        printf("job->auth_env[%d]=[%s],",i,job->auth_env[i]);
    }
    else
      break;
  ...
  printf("+++++++++++++++++++environment variable string create OK\n");
  ...
  */
  printf("-------------------------------begin to create processes for all filters\n");
       ...
       i ++, filter = (mime_filter_t *)cupsArrayNext(filters))
  {
    ...
    if (i < (cupsArrayCount(filters) - 1))
    {
      if (cupsdOpenPipe(filterfds[slot]))
      {
        abort_message = "Stopping job because the scheduler could not create "
	                "the filter pipes.";
        printf("Stopping job because the scheduler could not create the filter pipes.\n");
	...
	if (strncmp(job->printer->device_uri, "file:", 5) != 0)
	{
	  if (cupsdOpenPipe(job->print_pipes))
	  {
	    abort_message = "Stopping job because the scheduler could not "
            ...
            printf("Stopping job because the scheduler could not create the backend pipes.\n");
            goto abort_job;
	  }
	}
	else
	  ...
	  else
	  {
	    ...
	    if (job->print_pipes[1] < 0)
	    {
	      abort_message = "Stopping job because the scheduler could not "
              ...
              printf("Stopping job because the scheduler could not open the output file.\n");
              goto abort_job;
	    }
    ...
    if (pid == 0)
    {
      ...
      abort_message = "Stopping job because the scheduler could not execute a "
		      "filter.";
      printf( "Stopping job because the scheduler could not execute a filter.\n");
    ...
    argv[6] = NULL;
    slot    = !slot;
    printf("Started filter %s (PID %d)\n", command,pid);
  }

  cupsArrayDelete(filters);
  filters = NULL;
  printf("-------------------------------create processes for all filters OK\n");
 /*
  ...
  */
  ...
  if (strncmp(job->printer->device_uri, "file:", 5) != 0)
  {
        ...
        (job->printer->pc && job->printer->pc->single_file))
    {
      ...
      printf("backend directory =%s/backend/%s\n", ServerBin, scheme);
	...
	abort_message = "Stopping job because the sheduler could not execute "
			"the backend.";
        printf("Stopping job because the sheduler could not execute the backend.\n");
        goto abort_job;
      }
      else
      {
	cupsdLogJob(job, CUPSD_LOG_INFO, "Started backend %s (PID %d)",
		    command, pid);
        printf("Started backend %s (PID %d)\n",command, pid);
      }
    }
    ...
    if (job->current_file == job->num_files ||
        (job->printer->pc && job->printer->pc->single_file))
    {   
        printf("close print_pipes\n");
        cupsdClosePipe(job->print_pipes);
    }

    if (job->current_file == job->num_files)
    {
      printf("close back_pipes and side_pipes\n");
 ...
 printf("abort to update the printer and job state\n");
...
cupsdLoadAllJobs(void)
{
  ...
  printf("Into cupsdLoadAllJobs Function\n");
 /*
...
cupsdLoadJob(cupsd_job_t *job)		/* I - Job */
{
 ...
 printf("[Job %d] Loading attributes...\n", job->id);
  ...
  */
  ...
  if (job->state_value < IPP_JOB_STOPPED)
  {
    ...
    printf("Loac authentication infro,jobfile=%s\n",jobfile);
...
cupsdSaveAllJobs(void)
{
  ...
  printf("Into cupsdSaveAllJobs Function to save job.cache\n");
  snprintf(filename, sizeof(filename), "%s/job.cache", CacheDir);
...
cupsdSaveJob(cupsd_job_t *job)		/* I - Job */
{
  ...
  printf("Into cupsdSaveJob Function to save c0000x file\n");
  ...
  { 
      printf("cupsdCreate %s return NUll\n",filename);
      return;
  }
  fchown(cupsFileNumber(fp), RunUser, Group);
  printf("createConfFile,and fchown RunUser,Group\n");
		     ...
		     int         update)/* I - Update job-hold-until attr? */
{
  ...
  printf("Into cupsdSetJobHoldUntil Function to set job->hold_until time with argv when = %s,update = %d\n",when,update);
    ...
    int         priority)		/* I - New priority (0 to 100) */
{
 ...
 printf("Into cupsdSetJobPriority Function to set job Priority\n");
    ...
    ...)				/* I - Additional arguments as needed */
{
  ...
  printf("Into cuspdSetJobState Function to set job state\n");
    ...
    int               kill_delay)	/* I - Number of seconds before we kill */
{
  ...
  printf("Into cupsdStopAllJobs Function\n");
...
cupsdUnloadCompletedJobs(void)
{
 ...
 // printf("Into cupsdUnloadCompletedJobs Function to flush completed job history from memory\n");
  expire = time(NULL) - 60;
...
cupsdUpdateJobs(void)
{
  ...
  printf("Into cupsdUpdateJobs Function to update the history files for all jobs\n");
		    ...
		    void *data)		/* I - App data (not used) */
{
  ...
  printf("Into compare_active_jobs Function to compare the job IDs and priorities of two jobs\n");
  (void)data;
	     ...
	     void *data)		/* I - App data (not used) */
{
  ...
  printf("Into compare_jobs Function to compare the job IDs of two jobs\n");
...
dump_job_history(cupsd_job_t *job)	/* I - Job */
{
  ...
  printf("Into dump_job_history Function to Dump any debug msg for a job\n");
 /*
...
free_job_history(cupsd_job_t *job)	/* I - Job */
{
  ...
  printf("Into free_job_history Function to Free any log history\n");
             ...
             int         set_job_state)	/* I - 1 = set the job state */
{
  ...
  printf("Into finalize_job Function to cleanup after job filter process and support data\n");
  ...
  */
  ...
  if (job->status < 0)
  {
    ...
    */
    ...
    printf("Backend returned status %d (%s)",
		exit_code,
		exit_code == CUPS_BACKEND_FAILED ? "failed" :
		    exit_code == CUPS_BACKEND_AUTH_REQUIRED ?
			"authentication required" :
	    ...
	    size_t      title_size)	/* I - Size of title buffer */
{
  ...
  */
  ...
  printf("Into get_options Function to get a string that containing the job options\n");
...
ipp_length(ipp_t *ipp)			/* I - IPP request */
{
  ...
  */
  ...
  printf("Into ipp_length Function to Compute the size of the buffer needed to hold the textual IPP attributes\n");
  return (bytes);
}
...
load_job_cache(const char *filename)	/* I - job.cache filename */
{
  ...
  printf("Into load_job_cache Function to Load jobs from the job.cache file\n");
...
load_next_job_id(const char *filename)	/* I - job.cache filename */
{
  ...
  printf("Into load_next_job_id Function to load the NextJobId value from the job.cache file\n");
...
load_request_root(void)
{
  ...
  printf("Into load_request_root Function to load jobs from the RequestRoot directory\n");
...
remove_job_files(cupsd_job_t *job)	/* I - Job */
{
  ...
  printf("Into remove_job_files Function to remove the document files for a job\n");
...
remove_job_history(cupsd_job_t *job)	/* I - Job */
{
  ...
  */
  printf("Into remove_job_history Function to remove the control file for a job\n");
         ...
         const char  *name)		/* I - Name of attribute */
{
  ...
  printf("Into set_time Function to set one of the time-at-xyz attributes\n");
          ...
          cupsd_printer_t *printer)	/* I - Printer to print job */
{
  ...
  printf("=================================>Into start_job function\n");
  ...
  */
  ...
  if (job->num_files == 0)
  {
    ...
    printf("numfile == 0,set jobstate Abort\n");
    return;
  }
  ...
  */
  ...
  if (!cupsdLoadJob(job))
  {
  	printf("cupsdLoadJob failed ,return\n");
  	  return;
  }
  ...
  */
  ...
  if (cupsdOpenPipe(job->status_pipes))
  {
    ...
    cupsdDestroyProfile(job->profile);
    job->profile = NULL;
    printf("cupsdOpen status pipes failed,return\n");
    return;
  }
  ...
  */
  ...
  if (cupsdOpenPipe(job->back_pipes))
  {
    ...
    cupsdDestroyProfile(job->profile);
    job->profile = NULL;
    printf("create backchannel pipe failed,return \n");
    return;
  }
  ...
  */
  ...
  if (socketpair(AF_LOCAL, SOCK_STREAM, 0, job->side_pipes))
  {
    ...
    cupsdDestroyProfile(job->profile);
    job->profile = NULL;
    printf("create side-channel pipes failed,return\n");
    return;
  }
  ...
  */
  ...
  cupsdContinueJob(job);
  printf("<=================================Leave start_job function\n");
}
         ...
         cupsd_jobaction_t action)	/* I - Action */
{
  ...
  printf("Into stop_job Function to stop a print job\n");
...
unload_job(cupsd_job_t *job)		/* I - Job */
{
  ...
  printf("Into unload_job Function to unload a job from memory\n");
...
update_job(cupsd_job_t *job)		/* I - Job to check */
{
  ...
  printf("Into update_job Function to read a status update from a job's filters\n");
                 ...
                 int         do_message)/* I - 1 = copy job-printer-state message */
{
  ...
  printf("Into update_job_attrs Function to update the  job-printer-* attributes\n");

...
cupsdReadConfiguration(void)
{
  ...
  printf("cupsd into ReadConfiguration,ServerRoot=[%s],RequestRoot=[%s]\n",ServerRoot,RequestRoot);
 /*
...
printf("ServerBin--->[%s]\n \
          ...
          ServerRoot-->[%s]\n \
          ConfigurationFile-->[%s]\n \
          PrintCapFormat -->[%d]\n",ServerBin,RequestRoot,CacheDir,DataDir,DocumentRoot,Printcap,StateDir,ServerRoot,ConfigurationFile,PrintcapFormat);
  ...
  */
  ...
  printf("LPDConfigFile-->[%s]\n \
          SMBConfigFile-->[%s]\n",LPDConfigFile,SMBConfigFile);
...
#  endif /* HAVE_LIBSSL || HAVE_GNUTLS */
  ...
  */
  ...
  printf("cupsdReadConfiguration: NumPolicies=%d\n",
                  cupsArrayCount(Policies));
  for (i = 0, p = (cupsd_policy_t *)cupsArrayFirst(Policies);
    ...
    printf("cupsdReadConfiguration: Policies[%d]=\"%s\"", i, p->name);
    }
 /*
  ...
  */
      ...
      !old_requestroot || !RequestRoot || strcmp(old_requestroot, RequestRoot))
  {
    ...
    */
    ...
    printf("Read the MIME type and convs from %s, %s\n",ServerBin,DataDir);
    ...
    if (!MimeDatabase)
    {
      cupsdLogMessage(CUPSD_LOG_EMERG,
		      ...
		      mimedir, ServerRoot);
      printf("Unable to load MIME database from \"%s\" or \"%s\".\n",
		      mimedir, ServerRoot);
    ...
    printf("Loaded MIME database from \"%s\" and \"%s\": %d types, "
		    "%d filters...", mimedir, ServerRoot,
		    mimeNumTypes(MimeDatabase), mimeNumFilters(MimeDatabase));

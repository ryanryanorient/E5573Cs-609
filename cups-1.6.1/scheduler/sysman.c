...
{
  printf("Into cupsdCleanDirty Function\n");
  if (DirtyFiles & CUPSD_DIRTY_PRINTERS)
  {
      printf("DirtyFiles & CUPSD_DIRTY_PRINTERS\n");
      cupsdSaveAllPrinters();
  }

  if (DirtyFiles & CUPSD_DIRTY_CLASSES)
  {
      printf("DirtyFiles & CUPSD_DIRTY_CLASS\n");
      cupsdSaveAllClasses();
  }

  if (DirtyFiles & CUPSD_DIRTY_PRINTCAP)
  {
      printf("DirtyFiles & CUPSD_DIRTY_PRINTCAP\n");
      cupsdWritePrintcap();
  }
  if (DirtyFiles & CUPSD_DIRTY_JOBS)
  {
    cupsd_job_t	*job;			/* Current job */
    printf("DirtyFiles & CUPSD_DIRTY_JOBS\n");
    cupsdSaveAllJobs();
  ...
  {
      printf("DirtyFiles & CUPSD_DIRTY_SUBSCRIPTIONS\n");
...
cupsdMarkDirty(int what)		/* I - What file(s) are dirty? */
{
  ...
  printf("Into cupsdMarkDirty Function,cupsdMarkDirty(%c%c%c%c%c)\n", (what & CUPSD_DIRTY_PRINTERS) ? 'P' : '-',
		  (what & CUPSD_DIRTY_CLASSES) ? 'C' : '-',
		  (what & CUPSD_DIRTY_PRINTCAP) ? 'p' : '-',
		  (what & CUPSD_DIRTY_JOBS) ? 'J' : '-',
		  (what & CUPSD_DIRTY_SUBSCRIPTIONS) ? 'S' : '-');
...
//  printf("Into cupsdSetBusyState Function\n");
 /*

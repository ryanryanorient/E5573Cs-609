...
/*************************************************************************
问题单             修改人      日期       描述
DTS2013071806872  y00186923  2013-7-19 samba打印机上库
**************************************************************************/
#define _MAIN_C_
#include "cupsd.h"
...
/* BEGIN DTS2013071806872 y00186923 2013-07-24 added */					
/******************************************************************************
  函数名称  : FileAcquirePidFile
  功能描述  : 
              打开Pid的文件
  输入参数  : 
              1.  *pszPidFile:
  ...
  修改历史      :
   1.日    期   : 2013-7-22
     ...
     修改内容   : 完成初稿
******************************************************************************/
int FileAcquirePidFile(char *pszPidFile)
{
	int lPidFd;
	FILE* pFileOut;
	if (pszPidFile == NULL)
	{
        printf("pszPidFile is null\n");
        return -1;
	}
    
	lPidFd = open(pszPidFile, O_CREAT | O_WRONLY, 0644);
	if (lPidFd < 0) 
	{
		printf("Unable to open pszPidFile %s: %s\n",
		    pszPidFile, strerror(errno));
	} 
	else 
    {
    	if ((pFileOut = fdopen(lPidFd, "w")) != NULL)
    	{
    		flockfile(pFileOut);	
    	}
    	else
    	{
            printf("Unable to LOCK the pszPidFile %s: %s\n",
		    pszPidFile, strerror(errno));
    	}
    }
...
/******************************************************************************
  ...
  修改历史      :
   1.日    期   : 2013-7-22
     ...
     修改内容   : 完成初稿
******************************************************************************/
void FileWritePidFile(int lPidFd)
{
	FILE *pFileOut;
     
	if (lPidFd < 0) return;
    
    pFileOut = fdopen(lPidFd, "w");
        
	if (pFileOut != NULL)
    {
		fprintf(pFileOut, "%d\n", getpid());
        funlockfile(pFileOut);
		fclose(pFileOut);
	}
...
/* END DTS2013071806872 y00186923 2013-07-24 added */	
/*
 * 'main()' - Main entry for the CUPS scheduler.
 */
     ...
     char *argv[])			/* I - Command-line arguments */
{
  ...
  */
  ...
  {  
      printf("cupsd set ConfigurationFile =%s\n",CUPS_SERVERROOT "/cupsd.conf");
  ...
  */
  ...
  if (!fg)
  {
    ...
    */
    printf("not specified -f,run in the background\n");
    ...
    if (fork() > 0)
    {
      ...
      */
      printf("fork > 0,wait for the child to startup and send SIGUSR1\n");
  ...
  */
  ...
  {
    if (TestConfigFile)
      printf("TestConfigFile == true,%s contains errors\n", ConfigurationFile);
    else
  ...
  {
    printf("TestConfigFile == true,%s is OK\n", ConfigurationFile);
    return (0);
  }
  ...
  */
  ...
  if (Launchd)
  {
    ...
    */
    printf("start by launchd and get listen socket\n");
    launchd_checkin();
    launchd_checkout();
  }
#endif /* HAVE_LAUNCHD */
  ...
  */
  ...
  httpInitialize();
  printf("httpinitialize OK\n");

  cupsdStartServer();
  printf("cupsdstartserver OK\n");
/* BEGIN DTS2013071806872 y00186923 2013-07-24 added */	
  int lPidFd;
  printf("FileAcquirePidFile ----------------------\n");
  lPidFd = FileAcquirePidFile("/var/cups/pid");
  FileWritePidFile(lPidFd);
  printf("FileAcquirePidFile ----------------------\n");
/* END DTS2013071806872 y00186923 2013-07-24 added */	
 /*
  ...
  */
  ...
  printf("HAVE_SIGACTION Defined\n");
#else
  signal(SIGCLD, sigchld_handler);	/* No, SIGCLD isn't a typo... */
  signal(SIGHUP, sighup_handler);
  signal(SIGPIPE, SIG_IGN);
  signal(SIGTERM, sigterm_handler);
  printf("NO,SIGCLD is not a typo\n");
#endif /* HAVE_SIGSET */
  ...
  */
  ...
  if (!fg || run_as_child)
  {
    printf("runing in bkground,signal parent process\n");
   /*
  ...
  */
  ...
  cupsdCheckJobs();
  printf("start any pending print jobs ok\n");
 /*
  * Loop forever...
  */
  ...
  printf("into cupsd schedule loop\n");
  while (!stop_scheduler)
  {
    ...
    */
    ...
    {
  //    printf("Update any pending documents.......\n");
      cupsdCheckJobs();
      senddoc_time = current_time;
    }

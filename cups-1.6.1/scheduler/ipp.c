    ...
    cupsd_client_t *con)		/* I - Client connection */
{
  ...
  printf("cupsdProcessIPPRequest(%p[%d]): operation_id = %04x\n",
  ...
  */
  ...
  else
  {
	...
	*/
        ...
        if (!language)
	{
	  ...
	  cupsdAddEvent(CUPSD_EVENT_SERVER_AUDIT, NULL, NULL,
        ...
        if (!uri)
	{
	  printf("Missing printer-uri, job-uri, or ppd-name " "attribute");
	...
	printf("Request attributes follow...\n");
	...
	*/
        ...
        if (uri)
	  printf("%s %s",ippOpString(con->request->request.op.operation_id),
			  uri->values[0].string.text);
        else
	  printf("%s",ippOpString(con->request->request.op.operation_id));

    printf("------ipp request operation_id=[%d],uri=[%s]\n",con->request->request.op.operation_id,uri);
            ...
            ipp_attribute_t *uri)	/* I - URI of printer */
{
  ...
  printf("add_printer(%p[%d], %s) name[%s],numvalue[%d]\n", con,
                  con->http.fd, uri->values[0].string.text,uri->name,uri->num_values);
  ...
  */
  ...
  httpSeparateURI(HTTP_URI_CODING_ALL, uri->values[0].string.text, scheme,
                  sizeof(scheme), username, sizeof(username), host,
		  sizeof(host), &port, resource, sizeof(resource));
 printf("separateURI scheme[%s],username[%s],host[%s],port[%d],resource[%s]\n",scheme,username,host,port,resource);
  ...
  */
  ...
  {  cupsdSetString(&printer->location, attr->values[0].string.text);
    printf("set printer->location->[%s]\n",attr->values[0].string.text);
  }
  if ((attr = ippFindAttribute(con->request, "printer-info",
                               IPP_TAG_TEXT)) != NULL)
  {  cupsdSetString(&printer->info, attr->values[0].string.text);
    printf("set printer->info->[%s]\n",attr->values[0].string.text);
  	}
  set_device_uri = 0;
                               ...
                               IPP_TAG_URI)) != NULL)
  {
   ...
   printf("have device-uri\n");
	...
	printf("separateuri attr uri[%s],scheme[%s],username[%s],host[%s],port[%d],resource[%s],uri_status[%d]\n",attr->values[0].string.text,scheme,username,host,port,resource,uri_status);
		  ...
		  ipp_attribute_t *uri)	/* I - Printer URI */
{
  ...
  printf("get_printer_attrs(%p[%d], %s)\n", con,
                  con->http.fd, uri->values[0].string.text);
  ...
  */
  ...
  cupsArrayDelete(ra);
  printf("get_printer_attrs return IPP_OK\n");
  con->response->request.status.status_code = IPP_OK;
}
             ...
             int            type)	/* I - 0 or CUPS_PRINTER_CLASS */
{
  ...
  */
  ...
  if ((status = cupsdCheckPolicy(DefaultPolicyPtr, con, NULL)) != HTTP_OK)
  {
    printf("cupsdCheckPolicy not return HTTP_OK\n");
    send_http_error(con, status, NULL);
    return;
  }
  ...
  */
  ...
  if (!Printers || !cupsArrayCount(Printers))
  {
    ...
    printf("Not Find destinations printer,return after send IPP_NOT_FIND\n");
    return;
  }
	 ...
	 ipp_attribute_t *uri)		/* I - Job URI */
{
  ...
  cupsdLogMessage(CUPSD_LOG_DEBUG2, "move_job(%p[%d], %s)", con, con->http.fd,
                  uri->values[0].string.text);
  printf("move_job(%p[%d], %s)", con, con->http.fd,
                  uri->values[0].string.text);
    ...
    ipp_attribute_t *uri)		/* I - Printer URI */
{
  ...
  cupsdLogMessage(CUPSD_LOG_DEBUG2, "release_held_new_jobs(%p[%d], %s)", con,
                  con->http.fd, uri->values[0].string.text);
  printf("release_held_new_jobs(%p[%d], %s)\n", con,
                  con->http.fd, uri->values[0].string.text);
    ...
    int            sub_id)		/* I - Subscription ID */
{
  ...
  cupsdLogMessage(CUPSD_LOG_DEBUG2,
                  ...
                  con, con->http.fd, sub_id);
  printf("renew_subscription(con=%p[%d], sub_id=%d)\n",
                  con, con->http.fd, sub_id);
    ...
    cupsd_printer_t *printer)		/* I - Printer, if any */
{
  ...
  cupsdLogMessage(status == HTTP_FORBIDDEN ? CUPSD_LOG_ERROR : CUPSD_LOG_DEBUG,
		  ...
		  uri ? uri->values[0].string.text : "no URI",
		  con->http.hostname);
  printf("Returning HTTP %s for %s (%s) from %s",
                  httpStatus(status),
		  con->request ?
		      ippOpString(con->request->request.op.operation_id) :
		      "no operation-id",
                  ...
                  ipp_attribute_t *uri)	/* I - Printer */
{
  ...
  cupsdLogMessage(CUPSD_LOG_DEBUG2, "set_printer_attrs(%p[%d], %s)", con,
                  con->http.fd, uri->values[0].string.text);
  printf("set_printer_attrs(%p[%d], %s)\n", con,
                  con->http.fd, uri->values[0].string.text);
              ...
              ipp_attribute_t *uri)	/* I - Printer URI */
{
  ...
  cupsdLogMessage(CUPSD_LOG_DEBUG2, "start_printer(%p[%d], %s)", con,
                  con->http.fd, uri->values[0].string.text);
  printf("start_printer(%p[%d], %s)", con,
                  con->http.fd, uri->values[0].string.text);
	     ...
	     ipp_attribute_t *uri)	/* I - Printer URI */
{
  ...
  */
                               ...
                               IPP_TAG_KEYWORD)) != NULL)
  {
      ...
      )
    {
      ...
      ippAddString(con->response, IPP_TAG_UNSUPPORTED_GROUP, IPP_TAG_KEYWORD,
	           "compression", NULL, attr->values[0].string.text);
      printf("validate_job---> cups not support compression\n");
      return;
    }
  }
  ...
  */
                                 ...
                                 IPP_TAG_MIMETYPE)) != NULL)
  {
               ...
               super, type) != 2)
    {
      send_ipp_status(con, IPP_BAD_REQUEST, _("Bad document-format \"%s\"."),
		      format->values[0].string.text);
      printf("validate-job--->cups not support Format\n");
      return;
    }
	...
	!mimeType(MimeDatabase, super, type))
    {
      ...
      ippAddString(con->response, IPP_TAG_UNSUPPORTED_GROUP, IPP_TAG_MIMETYPE,
                   "document-format", NULL, format->values[0].string.text);
      printf("validate-job--->cups unsupported document-format\n");
      return;
    }
  }
  ...
  */
  ...
  if (!cupsdValidateDest(uri->values[0].string.text, &dtype, &printer))
  {
    ...
    */
    ...
    send_ipp_status(con, IPP_NOT_FOUND,
                    _("The printer or class does not exist."));
    printf("validate-job--->the printer or class does not exist\n");
    return;
  }
  ...
  */
  ...
  {
    send_http_error(con, status, printer);
    printf("validate-job--->checkPolicy error\n");
    return;
  }
  else if (printer->num_auth_info_required == 1 &&
           !strcmp(printer->auth_info_required[0], "negotiate") &&
  ...
  {
    send_http_error(con, HTTP_UNAUTHORIZED, printer);
    printf("validate-job--->checkPolicy unauthorized\n");
    return;
  }
#ifdef HAVE_SSL
  else if (auth_info && !con->http.tls &&
  ...
  */
  ...
  con->response->request.status.status_code = IPP_OK;
  printf("validate-job--->everything is ok\n");
}
	      ...
	      int            userlen)	/* I - Length of username */
{
  ...
  cupsdLogMessage(CUPSD_LOG_DEBUG2,
   ...
   printf("validate_user(job=%d, con=%d, owner=\"%s\", username=%p, "
		  "userlen=%d)",
        	  job->id, con ? con->http.fd : 0,
		  owner ? owner : "(null)", username, userlen);

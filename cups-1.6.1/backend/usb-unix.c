	     ...
	     char	*argv[])	/* I - Command-line arguments */
{
  ...
  */
 system( "echo \"usb-unix.c into print_device\n\" >> /var/usblog");
  ...
  do
  {
    ...
    */
    ...
    snprintf(szcmd, sizeof(szcmd),"echo \"Disable backchannel data when use special printer,hostname=[%s],use_bc=[%d]\n\" >> /var/usblog",hostname,use_bc);
    system(szcmd);
	...
	*/
        ...
        _cupsLangPrintFilter(stderr, "INFO",
			     _("Unable to contact printer, queuing on next "
			       "printer in class."));
       system( "echo \"Unable to contact printer, queuing on next printer in class.\n\" >> /var/usblog");
	...
	*/
	...
	sleep(5);
      ...
      if (errno == EBUSY)
      {
        _cupsLangPrintFilter(stderr, "INFO", _("The printer is in use."));
         system( "echo \"The printer is in use.\n\" >> /var/usblog");
	sleep(10);
      }
	...
	_cupsLangPrintError("ERROR", _("Unable to open device file"));
      system( "echo \"Unable to open device file.\n\" >> /var/usblog");
	return (CUPS_BACKEND_FAILED);
      }
    }
  }
  ...
  fputs("STATE: -connecting-to-device\n", stderr);
  system( "echo \"STATE: -connecting-to-device.\n\" >> /var/usblog");
  ...
  while (copies > 0 && tbytes >= 0)
  {
    ...
    */
    ...
    if (print_fd != 0 && tbytes >= 0)
     ...
     memset(szcmd, 0 , sizeof(szcmd));
     snprintf(szcmd,sizeof(szcmd), "echo \"Print file sent [%d] byte.\n\" >> /var/usblog",tbytes);
     system(szcmd);
     }
  }
  ...
  */
  ...
  close(device_fd);
 system( "echo \"Close the USB port and return.\n\" >> /var/usblog");
  ...
  */
...
#ifdef __linux
  {
    ...
    */
    ...
    errno = ENODEV;
   system( "echo \"open_device return -1,usb:/dev/\n\" >> /var/usblog");
    return (-1);
  }
  else if (!strncmp(uri, "usb://", 6))
  {
    ...
    */
    system( "echo \"Find the correct USB device\n\" >> /var/usblog");
	system("chmod 777 -R /dev/usb");
    for (;;)
    {
	...
	*/
	...
	//if ((fd = open(device, O_RDWR | O_EXCL)) < 0 && errno == ENOENT)
	{
	  ...
	  if ((fd = open(device, O_RDWR | O_EXCL)) < 0 && errno == ENOENT)
	  {
	  	snprintf(szcmd, sizeof(szcmd), "echo \"%s %s %d error:%s\" >> /var/usblog", "to into backendgetDeviceID",device,i, strerror(errno));
	system(szcmd);
	    sprintf(device, "/dev/usb/usblp%d", i);
	...
	snprintf(szcmd, sizeof(szcmd), "echo \"2222%s %s %d error:%s\" >> /var/usblog", "to into backendgetDeviceID",device,i, strerror(errno));
	system(szcmd);
	if (fd >= 0)
	{
	  memset(szcmd,0,sizeof(szcmd));
	  snprintf(szcmd, sizeof(szcmd), "echo \"%s %s %d\" >> /var/usblog", "to into backendgetDeviceID",device,i);
	  system(szcmd);
	  backendGetDeviceID(fd, device_id, sizeof(device_id),
                             make_model, sizeof(make_model),
			     "usb", device_uri, sizeof(device_uri));
        memset(szcmd,0,sizeof(szcmd));
	  snprintf(szcmd, sizeof(szcmd), "echo \"device_id[%s] make_model=[%s] uri=[%s] device_uri=[%s]\" >> /var/usblog", 
	  device_id,make_model,uri,device_uri);
	  system(szcmd);
	}
	else
	{
        ...
        if (strcmp(uri, device_uri))
	{
	  ...
	  */
	  ...
	  fprintf(stderr, "DEBUG: Printer using device file \"%s\"...\n",
		  device);
         system( "echo \"return this file descriptor\n\" >> /var/usblog");
	  return (fd);
	}

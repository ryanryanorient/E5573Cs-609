	     ...
	     char	*argv[])	/* I - Command-line arguments */
{
  ...
  */
  ...
  printf("the usb.c  printf_device function return NULL,do nothing!\n");
     ...
     char *argv[])			/* I - Command-line arguments */
{
  ...
  */
  ...
  else if (argc < 6 || argc > 7)
  {
    _cupsLangPrintf(stderr,
                    ...
                    argv[0]);
    system( "echo \"usb.c argc < 6 or >7, error!\n\" >> /var/usblog");
    return (CUPS_BACKEND_FAILED);
  }
  ...
  */
		      ...
		      resource, sizeof(resource)) < HTTP_URI_OK)
  {
    ...
    printf("No device URI found in argv[0] or in DEVICE_URI environment variable.\n");
    system( "echo \"No device URI found in argv[0] or in DEVICE_URI environment variable.\n\" >> /var/usblog");
    return (1);
  }
  ...
  else
  {
    ...
    */
    ...
    if ((print_fd = open(argv[6], O_RDONLY)) < 0)
    {
      ...
      system( "echo \"usb.c to open file error!\n\" >> /var/usblog");
      return (CUPS_BACKEND_FAILED);
    }
  ...
  */
 system( "echo \"usb.c Try to send the print file\n\" >> /var/usblog");
  status = print_device(uri, hostname, resource, options, print_fd, copies,
                        argc, argv);
  memset(szcmd,0,sizeof(szcmd));
  sprintf(szcmd,"echo \"print_device return %d \n \">> /var/usblog",status);
  system(szcmd);
 /*
